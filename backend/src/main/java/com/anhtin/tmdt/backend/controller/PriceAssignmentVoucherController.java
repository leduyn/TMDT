...
import com.anhtin.tmdt.backend.dto.request.PriceAssignmentVoucherRequest;
import com.anhtin.tmdt.backend.dto.response.PriceAssignmentVoucherDTO;
...
public class PriceAssignmentVoucherController {
    ...
    private final com.anhtin.tmdt.backend.service.PriceAssignmentService priceAssignmentService;
    ...
    public ResponseEntity<?> cancelVoucher(@PathVariable @org.springframework.lang.NonNull Long id) {
        return voucherRepository.findById(id).map(v -> {
            if (v.getStatus() == com.anhtin.tmdt.backend.entity.VoucherStatus.PENDING) {
                v.setStatus(com.anhtin.tmdt.backend.entity.VoucherStatus.CANCELLED);
