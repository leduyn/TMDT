                            ...
                            <td className="p-3">
                              <div className="flex items-center gap-1">
                                <button
                                  ...
                                  className={`px-2 py-2.5 text-xs font-bold rounded-l-lg border transition-all ${t.adjustmentValue < 0
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleUpdateTierField(idx, 'adjustmentValue', Math.abs(t.adjustmentValue) || 1)}
                                  className={`px-2 py-2.5 text-xs font-bold border-y transition-all ${t.adjustmentValue > 0
            ...
            <div className={`grid grid-cols-1 ${isRetailMode ? 'max-w-md mx-auto' : 'md:grid-cols-2'} gap-8`}>
              ...
              <div className="flex flex-col items-center">
                ...
                >
                  ...
                  {/* Nội dung Mockup */}
                  <div className="flex-1 flex flex-col p-4 pt-8 overflow-y-auto text-[var(--text-primary)]">
                    {/* Top Header */}
                    ...
                    {/* 🚨 DYNAMIC BANNER CHÍNH SÁCH BÁN HÀNG 🚨 */}
                    {!isRetailMode && productPreview && productPreview.salesPolicies && productPreview.salesPolicies.length > 0 && (
                      <div
                        className="p-3 border rounded-xl mb-3 relative overflow-hidden"
                        style={{
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1 mb-1.5">
                          ...
                          <span className="text-[7px] font-bold text-indigo-300 uppercase tracking-wider">Ưu đãi Chính sách bán hàng</span>
                        </div>

                        <div className="flex flex-col gap-1.5">
                          {productPreview.salesPolicies.map(p => {
                            const basePrice = p.originalPrice || (previewProduct ? (previewProduct.appliedPrice || previewProduct.price) : 0);
                            let finalPrice = basePrice;
                            if (p.adjustmentType === 'PERCENTAGE') {
                              finalPrice = basePrice + (basePrice * (p.adjustmentValue || 0) / 100);
                            } else if (p.adjustmentType === 'FIXED_AMOUNT') {
                              finalPrice = basePrice + (p.adjustmentValue || 0);
                            } else if (p.adjustmentType === 'SPECIFIC_PRICE') {
                              finalPrice = p.adjustmentValue || 0;
                            }
                            finalPrice = Math.max(0, finalPrice);
                            return (
                              <div key={p.id} className="flex items-center justify-between text-[8px] bg-slate-950/40 p-1.5 rounded-lg border border-slate-900">
                                <div className="flex flex-col">
                                  <span className="text-white font-semibold">{p.name}</span>
                                  {p.conditionText && <span className="text-[6px] text-slate-500 mt-0.5">{p.conditionText}</span>}
                                </div>
                                <div className="flex items-center gap-1">
                                  <span className={`font-bold ${(p.adjustmentValue || 0) < 0 ? 'text-emerald-400' : 'text-amber-400'}`}>
                                    {p.adjustmentValue < 0 ? `-${Math.abs(p.adjustmentValue)}` : `+${p.adjustmentValue}`}
                                    {p.adjustmentType === 'PERCENTAGE' ? '%' : 'đ'}
                                  </span>
                                  <span className="text-emerald-300 font-bold">→ {Math.round(finalPrice).toLocaleString()}đ</span>
                                  {p.giftProductName && (
                                    <span className="bg-emerald-950/40 text-emerald-300 px-1 rounded-md" style={{ fontSize: '7px' }}>🎁 {p.giftProductName}</span>
                                  )}
                                </div>
                              </div>
                            );
                    ...
                    {/* 🚨 DYNAMIC BANNER CHƯƠNG TRÌNH KHUYẾN MÃI 🚨 */}
                    {!isRetailMode && productPreview && productPreview.promotions && productPreview.promotions.length > 0 && (
                      <div
                        className="p-3 border rounded-xl mb-3 relative overflow-hidden"
                        style={{
                          background: 'radial-gradient(circle at 0% 100%, rgba(16,185,129,0.15) 0%, rgba(26, 34, 53, 0.4) 80%)',
                          borderColor: 'rgba(16, 185, 129, 0.4)'
                        }}
                      >
                        <div className="absolute -bottom-6 -left-6 w-16 h-16 bg-emerald-500 opacity-10 rounded-full blur-xl"></div>

                        <div className="flex items-center gap-1 mb-1.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                          <span className="text-[7px] font-bold text-emerald-300 uppercase tracking-wider">Chương trình khuyến mãi</span>
                        </div>
                ...
                <div className="mt-8 w-full col-span-1 md:col-span-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {productPreview.wholesaleFlow && (() => {
                      ...
                      return (
                        <div
                          className={`glass-card p-5 relative overflow-hidden transition-all duration-300 ${isActive ? 'border-indigo-500/60 ring-2 ring-indigo-500/20 bg-indigo-950/5' : 'opacity-50 border-slate-800/60'
                            }`}
                    ...
                    {productPreview.retailFlow && (() => {
                      ...
                      return (
                        <div
                          className={`glass-card p-5 relative overflow-hidden transition-all duration-300 ${isActive ? 'border-amber-500/60 ring-2 ring-amber-500/20 bg-amber-950/5' : 'opacity-50 border-slate-800/60'
                            }`}
                          style={{ borderRadius: 16 }}
                        >
                          {isActive && (
              ...
              <div style={{
                ...
                <div style={{ padding: '14px 22px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 10, alignItems: 'center' }}>
                  ...
                  <button
                    ...
                    style={{
                      padding: '9px 14px', borderRadius: 10, fontSize: 11, fontWeight: 700, cursor: 'pointer', border: '1px solid', display: 'flex', alignItems: 'center', gap: 5, whiteSpace: 'nowrap',
